...
class Seed(db.Model):
    ...
    is_mature = db.mapped_column(db.Boolean, nullable=False, default=False)
    matured_time = db.mapped_column(db.DateTime, nullable = True, default = None)
 ...
 # return True or False whether the plant can be watered again
    ...
    def matured_time(self):
        if self.is_mature == None:
            self.matured_time = dt.now()
            return self.matured_time
